...
templates/knowledge-base/constraints.md
templates/knowledge-base/glossary.md
templates/knowledge-base/adr/
templates/knowledge-base/lessons/
```
...
条件文档（boundaries.md、flows.md）不在此步复制，由第四步根据项目需要判断是否创建。
...
```text
README.md         # 项目概览（系统定位、业务背景、项目目标、核心业务关系、项目阶段）（必填）
...
创建条件文档时，从 `templates/knowledge-base/` 中复制对应模板文件到目标目录，并替换 `{{KB_PATH}}` 占位符。

不需要时不要创建，避免空模板浪费 AI 注意力。
...
- 从代码中提取项目概览（系统定位、业务背景、项目目标、核心业务关系、项目阶段）
...
```text
□ README.md 项目概览是否已填写系统定位、业务背景、项目目标、核心业务关系
